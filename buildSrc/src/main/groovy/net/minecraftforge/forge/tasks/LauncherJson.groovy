...
abstract class LauncherJson extends DefaultTask {
    ...
    protected void exec() {
        if (!json.libraries)
            json.libraries = []
        def libs = [:]
        getArtifactsOffline(project, project.configurations.installer, false).each { key, lib -> libs[key] = lib }
        getArtifactsOffline(project, project.configurations.moduleonly, false).each { key, lib -> libs[key] = lib }
