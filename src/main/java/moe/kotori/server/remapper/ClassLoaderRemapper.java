...
import moe.kotori.server.remapper.ClassLoaderRepo;
import moe.kotori.server.remapper.GlobalClassRepo;
import moe.kotori.server.remapper.LenientJarRemapper;
import moe.kotori.server.remapper.PluginTransformer;
...
public class ClassLoaderRemapper extends LenientJarRemapper {
    ...
    public ClassLoaderRemapper(JarMapping jarMapping, JarMapping toBukkitMapping, ClassLoader classLoader) {
        ...
        this.jarMapping.setInheritanceMap(LoliServerRemapper.INSTANCE.inheritanceMap);
    ...
    public Map.Entry<String, String> tryClimb(Map<String, String> map, String owner, String name, int access) {
        ...
        if (mapped == null && (access == -1 || (!Modifier.isPrivate(access) && !Modifier.isStatic(access)))) {
            ...
            if (LoliServerRemapper.INSTANCE.inheritanceMap.hasParents(owner)) {
                parents = LoliServerRemapper.INSTANCE.inheritanceMap.getParents(owner);
            } else {
                parents = GlobalClassRepo.inheritanceProvider().getParents(owner);
                LoliServerRemapper.INSTANCE.inheritanceMap.setParents(owner, parents);
            }
    ...
    private byte[] remapClassFile(ClassReader reader, final ClassRepo repo) {
        ...
        for (PluginTransformer transformer : LoliServerRemapper.INSTANCE.getTransformerList()) {
    ...
    private static class PluginClassWriter extends ClassWriter {
        ...
        private String getSuper(final String typeName) {
            ...
            return LoliServerRemapper.getNmsMapper().map(node.superName);
        }
    }
    ...
    private static byte[] dump(byte[] bytes) {
        try {
            if (LoliServerRemapper.DUMP != null) {
                String className = new ClassReader(bytes).getClassName() + ".class";
                int index = className.lastIndexOf('/');
                if (index != -1) {
                    File file = new File(LoliServerRemapper.DUMP, className.substring(0, index));
                    file.mkdirs();
                    Files.write(file.toPath().resolve(className.substring(index + 1)), bytes);
                } else {
                    Files.write(LoliServerRemapper.DUMP.toPath().resolve(className), bytes);
                }
            }
        } catch (Exception e) {
