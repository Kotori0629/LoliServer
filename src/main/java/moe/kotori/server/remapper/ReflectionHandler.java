...
import moe.kotori.server.utils.Enumerations;
...
public class ReflectionHandler extends ClassLoader {
    ...
    public static String handlePackageGetName(String name) {
        if (name.startsWith(PREFIX)) {
            return PREFIX + "server." + LoliServer.NATIVE_VERSION;
        } else {
            return name;
        }
    }
    ...
    public static byte[] transformOrAdd(ClassLoader loader, byte[] bytes) {
        RemapperClassLoader rcl = null;
        while (loader != null) {
            if (loader instanceof RemapperClassLoader) {
                rcl = ((RemapperClassLoader) loader);
                break;
            } else {
