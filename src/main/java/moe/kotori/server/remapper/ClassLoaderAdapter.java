...
import moe.kotori.server.remapper.ClassLoaderRemapper;
...
public class ClassLoaderAdapter implements PluginTransformer {
    ...
    private final Map<String, String> classLoaderTypes = ImmutableMap.<String, String>builder()
            .put(Type.getInternalName(URLClassLoader.class), Type.getInternalName(ProxyURLClassLoader.class))
            .build();
    ...
    private void implementIntf(ClassNode node) {
        ...
        list.add(new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(LoliServerRemapper.class), "createClassLoaderRemapper", Type.getMethodDescriptor(Type.getType(ClassLoaderRemapper.class), Type.getType(ClassLoader.class)), false));
        ...
        node.interfaces.add(Type.getInternalName(RemapperClassLoader.class));
    }
